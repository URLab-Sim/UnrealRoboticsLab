...
void UMjActuator::ExportTo(mjsActuator* Element, mjsDefault* Default)
{
	...
	switch (TransmissionType)
	{
		case EMjActuatorTrnType::Joint:
			Element->trntype = mjTRN_JOINT;
			break;
		case EMjActuatorTrnType::JointInParent:
			Element->trntype = mjTRN_JOINTINPARENT;
			break;
		case EMjActuatorTrnType::SliderCrank:
			Element->trntype = mjTRN_SLIDERCRANK;
			break;
		case EMjActuatorTrnType::Tendon:
			Element->trntype = mjTRN_TENDON;
			break;
		case EMjActuatorTrnType::Site:
			Element->trntype = mjTRN_SITE;
			break;
		case EMjActuatorTrnType::Body:
			Element->trntype = mjTRN_BODY;
			break;
		default:
			Element->trntype = mjTRN_UNDEFINED;
			break;
	}
	...
	if (bOverride_GainType)
	{
		switch (GainType)
		{
			case EMjGainType::Fixed:
				Element->gaintype = (mjtGain)mjGAIN_FIXED;
				break;
			case EMjGainType::Affine:
				Element->gaintype = (mjtGain)mjGAIN_AFFINE;
				break;
			case EMjGainType::Muscle:
				Element->gaintype = (mjtGain)mjGAIN_MUSCLE;
				break;
			case EMjGainType::User:
				Element->gaintype = (mjtGain)mjGAIN_USER;
				break;
			default:
				break;
		}
	...
	if (bOverride_BiasType)
	{
		switch (BiasType)
		{
			case EMjBiasType::None:
				Element->biastype = (mjtBias)mjBIAS_NONE;
				break;
			case EMjBiasType::Affine:
				Element->biastype = (mjtBias)mjBIAS_AFFINE;
				break;
			case EMjBiasType::Muscle:
				Element->biastype = (mjtBias)mjBIAS_MUSCLE;
				break;
			case EMjBiasType::User:
				Element->biastype = (mjtBias)mjBIAS_USER;
				break;
			default:
				break;
		}
	...
	if (bOverride_DynType)
	{
		switch (DynType)
		{
			case EMjDynType::None:
				Element->dyntype = (mjtDyn)mjDYN_NONE;
				break;
			case EMjDynType::Integrator:
				Element->dyntype = (mjtDyn)mjDYN_INTEGRATOR;
				break;
			case EMjDynType::Filter:
				Element->dyntype = (mjtDyn)mjDYN_FILTER;
				break;
			case EMjDynType::FilterExact:
				Element->dyntype = (mjtDyn)mjDYN_FILTEREXACT;
				break;
			case EMjDynType::Muscle:
				Element->dyntype = (mjtDyn)mjDYN_MUSCLE;
				break;
			case EMjDynType::User:
				Element->dyntype = (mjtDyn)mjDYN_USER;
				break;
			default:
				break;
		}
	...
	if (bOverride_delay)
		Element->delay = delay;
	if (bOverride_ctrllimited)
		Element->ctrllimited = ctrllimited ? 1 : 0;
	if (bOverride_forcelimited)
		Element->forcelimited = forcelimited ? 1 : 0;
	if (bOverride_actlimited)
		Element->actlimited = actlimited ? 1 : 0;
	if (bOverride_ctrlrange)
	{
		for (int32 i = 0; i < FMath::Min(ctrlrange.Num(), 2); ++i)
			Element->ctrlrange[i] = ctrlrange[i];
	}
	if (bOverride_forcerange)
	{
		for (int32 i = 0; i < FMath::Min(forcerange.Num(), 2); ++i)
			Element->forcerange[i] = forcerange[i];
	}
	if (bOverride_actrange)
	{
		for (int32 i = 0; i < FMath::Min(actrange.Num(), 2); ++i)
			Element->actrange[i] = actrange[i];
	}
	if (bOverride_lengthrange)
	{
		for (int32 i = 0; i < FMath::Min(lengthrange.Num(), 2); ++i)
			Element->lengthrange[i] = lengthrange[i];
	}
	if (bOverride_gear)
	{
		for (int32 i = 0; i < FMath::Min(gear.Num(), 6); ++i)
			Element->gear[i] = gear[i];
	}
	...
	if (bOverride_cranklength)
		Element->cranklength = cranklength;
	if (bOverride_gainprm)
	{
		for (int32 i = 0; i < FMath::Min(gainprm.Num(), 10); ++i)
			Element->gainprm[i] = gainprm[i];
	}
	if (bOverride_biasprm)
	{
		for (int32 i = 0; i < FMath::Min(biasprm.Num(), 10); ++i)
			Element->biasprm[i] = biasprm[i];
	}
	if (bOverride_dynprm)
	{
		for (int32 i = 0; i < FMath::Min(dynprm.Num(), 10); ++i)
			Element->dynprm[i] = dynprm[i];
	}
	if (bOverride_actdim)
		Element->actdim = actdim;
	// --- CODEGEN_EXPORT_END ---
}
...
void UMjActuator::ImportFromXml(const FXmlNode* Node, const FMjCompilerSettings& CompilerSettings)
{
	...
	{ // xml_enum: gaintype -> EMjGainType
		...
		if (S == TEXT("fixed"))
		{
			GainType = EMjGainType::Fixed;
			bMatched = true;
		}
		else if (S == TEXT("affine"))
		{
			GainType = EMjGainType::Affine;
			bMatched = true;
		}
		else if (S == TEXT("muscle"))
		{
			GainType = EMjGainType::Muscle;
			bMatched = true;
		}
		else if (S == TEXT("user"))
		{
			GainType = EMjGainType::User;
			bMatched = true;
		}
		if (bMatched)
			bOverride_GainType = true;
	}
	{ // xml_enum: biastype -> EMjBiasType
		...
		if (S == TEXT("none"))
		{
			BiasType = EMjBiasType::None;
			bMatched = true;
		}
		else if (S == TEXT("affine"))
		{
			BiasType = EMjBiasType::Affine;
			bMatched = true;
		}
		else if (S == TEXT("muscle"))
		{
			BiasType = EMjBiasType::Muscle;
			bMatched = true;
		}
		else if (S == TEXT("user"))
		{
			BiasType = EMjBiasType::User;
			bMatched = true;
		}
		if (bMatched)
			bOverride_BiasType = true;
	}
	{ // xml_enum: dyntype -> EMjDynType
		...
		if (S == TEXT("none"))
		{
			DynType = EMjDynType::None;
			bMatched = true;
		}
		else if (S == TEXT("integrator"))
		{
			DynType = EMjDynType::Integrator;
			bMatched = true;
		}
		else if (S == TEXT("filter"))
		{
			DynType = EMjDynType::Filter;
			bMatched = true;
		}
		else if (S == TEXT("filterexact"))
		{
			DynType = EMjDynType::FilterExact;
			bMatched = true;
		}
		else if (S == TEXT("muscle"))
		{
			DynType = EMjDynType::Muscle;
			bMatched = true;
		}
		else if (S == TEXT("user"))
		{
			DynType = EMjDynType::User;
			bMatched = true;
		}
		if (bMatched)
			bOverride_DynType = true;
	}
...
TArray<float> UMjActuator::GetGear() const
{
	...
	{
		for (int i = 0; i < 6; ++i)
			Res.Add((float)m_ActuatorView.actuator_gear[i]);
	}
	return Res;
}
...
{
	if (m_ActuatorView.id < 0 || !m_ActuatorView.actuator_ctrlrange)
...
{
	if (m_ActuatorView.id < 0 || !m_ActuatorView.act)
		return 0.0f;
...
{
	if (m_ActuatorView.id < 0 || !m_ActuatorView.name)
		return FString();
...
{
	return UMjComponent::GetSiblingComponentOptions(this, UMjSite::StaticClass());
}

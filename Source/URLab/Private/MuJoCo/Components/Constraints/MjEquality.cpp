...
void UMjEquality::ImportFromXml(const FXmlNode* Node, const FMjCompilerSettings& CompilerSettings)
{
	...
	// (Weld) and silently misbehave for joint/tendon/flex equalities.
	{
		...
		if (Tag == TEXT("connect"))
			EqualityType = EMjEqualityType::Connect;
		else if (Tag == TEXT("weld"))
			EqualityType = EMjEqualityType::Weld;
		else if (Tag == TEXT("joint"))
			EqualityType = EMjEqualityType::Joint;
		else if (Tag == TEXT("tendon"))
			EqualityType = EMjEqualityType::Tendon;
		else if (Tag == TEXT("flex"))
			EqualityType = EMjEqualityType::Flex;
		else if (Tag == TEXT("flexvert"))
			EqualityType = EMjEqualityType::FlexVert;
		else if (Tag == TEXT("flexstrain"))
			EqualityType = EMjEqualityType::FlexStrain;
	}
	...
	if (MjXmlUtils::ReadAttrString(Node, TEXT("site1"), site1))
		bOverride_site1 = true;
	if (MjXmlUtils::ReadAttrString(Node, TEXT("site2"), site2))
		bOverride_site2 = true;
	...
	if (Obj1.IsEmpty())
		Obj1 = Node->GetAttribute(TEXT("site1"));
	if (Obj1.IsEmpty())
		Obj1 = Node->GetAttribute(TEXT("joint1"));
	if (Obj1.IsEmpty())
		Obj1 = Node->GetAttribute(TEXT("tendon1"));
	if (Obj1.IsEmpty())
		Obj1 = Node->GetAttribute(TEXT("flex"));
	// target_collation: -> Obj2
	Obj2 = Node->GetAttribute(TEXT("body2"));
	if (Obj2.IsEmpty())
		Obj2 = Node->GetAttribute(TEXT("site2"));
	if (Obj2.IsEmpty())
		Obj2 = Node->GetAttribute(TEXT("joint2"));
	if (Obj2.IsEmpty())
		Obj2 = Node->GetAttribute(TEXT("tendon2"));
	if (bOverride_anchor)
	{
		if ((EqualityType == EMjEqualityType::Connect) || (EqualityType == EMjEqualityType::Weld))
		{
			for (float& V : anchor)
			{
				V *= 100.0f;
...
UMjConnectEquality::UMjConnectEquality()
{
	EqualityType = EMjEqualityType::Connect;
}
UMjWeldEquality::UMjWeldEquality()
{
	EqualityType = EMjEqualityType::Weld;
}
UMjJointEquality::UMjJointEquality()
{
	EqualityType = EMjEqualityType::Joint;
}
UMjTendonEquality::UMjTendonEquality()
{
	EqualityType = EMjEqualityType::Tendon;
}
UMjFlexEquality::UMjFlexEquality()
{
	EqualityType = EMjEqualityType::Flex;
}
UMjFlexVertEquality::UMjFlexVertEquality()
{
	EqualityType = EMjEqualityType::FlexVert;
}
UMjFlexStrainEquality::UMjFlexStrainEquality()
{
	EqualityType = EMjEqualityType::FlexStrain;
}

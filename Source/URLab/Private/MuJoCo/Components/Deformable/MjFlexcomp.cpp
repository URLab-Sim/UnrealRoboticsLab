...
void UMjFlexcomp::ImportFromXml(const FXmlNode* Node, const FMjCompilerSettings& CompilerSettings)
{
	...
	{ // xml_enum: type -> EMjFlexcompType
		...
		if (S == TEXT("grid"))
		{
			FlexcompType = EMjFlexcompType::Grid;
			bMatched = true;
		}
		else if (S == TEXT("box"))
		{
			FlexcompType = EMjFlexcompType::Box;
			bMatched = true;
		}
		else if (S == TEXT("cylinder"))
		{
			FlexcompType = EMjFlexcompType::Cylinder;
			bMatched = true;
		}
		else if (S == TEXT("ellipsoid"))
		{
			FlexcompType = EMjFlexcompType::Ellipsoid;
			bMatched = true;
		}
		else if (S == TEXT("square"))
		{
			FlexcompType = EMjFlexcompType::Square;
			bMatched = true;
		}
		else if (S == TEXT("disc"))
		{
			FlexcompType = EMjFlexcompType::Disc;
			bMatched = true;
		}
		else if (S == TEXT("circle"))
		{
			FlexcompType = EMjFlexcompType::Circle;
			bMatched = true;
		}
		else if (S == TEXT("mesh"))
		{
			FlexcompType = EMjFlexcompType::Mesh;
			bMatched = true;
		}
		else if (S == TEXT("gmsh"))
		{
			FlexcompType = EMjFlexcompType::Gmsh;
			bMatched = true;
		}
		else if (S == TEXT("direct"))
		{
			FlexcompType = EMjFlexcompType::Direct;
			bMatched = true;
		}
		if (bMatched)
			bOverride_FlexcompType = true;
	}
	{ // xml_enum: dof -> EMjFlexcompDof
		...
		if (S == TEXT("full"))
		{
			FlexcompDof = EMjFlexcompDof::Full;
			bMatched = true;
		}
		else if (S == TEXT("radial"))
		{
			FlexcompDof = EMjFlexcompDof::Radial;
			bMatched = true;
		}
		else if (S == TEXT("trilinear"))
		{
			FlexcompDof = EMjFlexcompDof::Trilinear;
			bMatched = true;
		}
		else if (S == TEXT("quadratic"))
		{
			FlexcompDof = EMjFlexcompDof::Quadratic;
			bMatched = true;
		}
		else if (S == TEXT("2d"))
		{
			FlexcompDof = EMjFlexcompDof::TwoD;
			bMatched = true;
		}
		if (bMatched)
			bOverride_FlexcompDof = true;
	}
	...
	if (MjXmlUtils::ReadAttrString(Node, TEXT("file"), file))
		bOverride_file = true;
	...
	for (const FXmlNode* Child : Node->GetChildrenNodes())
	{
		...
		if (ChildTag == TEXT("contact"))
		{
			...
			if (!ConTypeStr.IsEmpty())
			{
				ConType = FCString::Atoi(*ConTypeStr);
				bOverride_ConType = true;
			}

			FString ConAffStr = Child->GetAttribute(TEXT("conaffinity"));
			if (!ConAffStr.IsEmpty())
			{
				ConAffinity = FCString::Atoi(*ConAffStr);
				bOverride_ConAffinity = true;
			}

			FString ConDimStr = Child->GetAttribute(TEXT("condim"));
			if (!ConDimStr.IsEmpty())
			{
				ConDim = FCString::Atoi(*ConDimStr);
				bOverride_ConDim = true;
			}

			FString PriorityStr = Child->GetAttribute(TEXT("priority"));
			if (!PriorityStr.IsEmpty())
			{
				Priority = FCString::Atoi(*PriorityStr);
				bOverride_Priority = true;
			}
			...
			if (!SelfStr.IsEmpty())
			{
				if (SelfStr == TEXT("none"))
					SelfCollide = 0;
				else if (SelfStr == TEXT("auto"))
					SelfCollide = 1;
				else if (SelfStr == TEXT("all"))
					SelfCollide = 2;
			...
			if (!IntStr.IsEmpty())
			{
				bInternal = IntStr.ToBool();
				bOverride_Internal = true;
			}
		...
		else if (ChildTag == TEXT("edge"))
		{
			...
			if (!EqStr.IsEmpty())
			{
				bEdgeEquality = EqStr.ToBool();
				bOverride_EdgeEquality = true;
			}
		...
		else if (ChildTag == TEXT("elasticity"))
		{
			...
			if (!E2DStr.IsEmpty())
			{
				if (E2DStr == TEXT("none"))
					Elastic2D = 0;
				else if (E2DStr == TEXT("bend"))
					Elastic2D = 1;
				else if (E2DStr == TEXT("stretch"))
					Elastic2D = 2;
				else if (E2DStr == TEXT("both"))
					Elastic2D = 3;
		...
		else if (ChildTag == TEXT("pin"))
		{
			...
			if (!GridRangeStr.IsEmpty())
			{
				...
				for (const FString& P : Parts)
					PinGridRange.Add(FCString::Atoi(*P));
			}
		}
	}
...
FString UMjFlexcomp::ExportMeshToVFS(FMujocoSpecWrapper& Wrapper)
{
	...
	for (int32 i = 0; i < NumRawVerts; i++)
	{
		...
		{
			if (UniquePositions[C].Equals(LocalPos, WeldTolerance))
			{
				Found = C;
				break;
			}
...
FString UMjFlexcomp::BuildSchemaAttrsXml() const
{
	...
	{
		if (FlexcompType == EMjFlexcompType::Grid)
			Out += TEXT(" type=\"grid\"");
		else if (FlexcompType == EMjFlexcompType::Box)
			Out += TEXT(" type=\"box\"");
		else if (FlexcompType == EMjFlexcompType::Cylinder)
			Out += TEXT(" type=\"cylinder\"");
		else if (FlexcompType == EMjFlexcompType::Ellipsoid)
			Out += TEXT(" type=\"ellipsoid\"");
		else if (FlexcompType == EMjFlexcompType::Square)
			Out += TEXT(" type=\"square\"");
		else if (FlexcompType == EMjFlexcompType::Disc)
			Out += TEXT(" type=\"disc\"");
		else if (FlexcompType == EMjFlexcompType::Circle)
			Out += TEXT(" type=\"circle\"");
		else if (FlexcompType == EMjFlexcompType::Mesh)
			Out += TEXT(" type=\"mesh\"");
		else if (FlexcompType == EMjFlexcompType::Gmsh)
			Out += TEXT(" type=\"gmsh\"");
		else if (FlexcompType == EMjFlexcompType::Direct)
			Out += TEXT(" type=\"direct\"");
	}
	if (bOverride_FlexcompDof)
	{
		if (FlexcompDof == EMjFlexcompDof::Full)
			Out += TEXT(" dof=\"full\"");
		else if (FlexcompDof == EMjFlexcompDof::Radial)
			Out += TEXT(" dof=\"radial\"");
		else if (FlexcompDof == EMjFlexcompDof::Trilinear)
			Out += TEXT(" dof=\"trilinear\"");
		else if (FlexcompDof == EMjFlexcompDof::Quadratic)
			Out += TEXT(" dof=\"quadratic\"");
		else if (FlexcompDof == EMjFlexcompDof::TwoD)
			Out += TEXT(" dof=\"2d\"");
	}
	if (bOverride_group)
		Out += FString::Printf(TEXT(" group=\"%d\""), group);
	if (bOverride_dim)
		Out += FString::Printf(TEXT(" dim=\"%d\""), dim);
	...
	if (bOverride_radius)
		Out += FString::Printf(TEXT(" radius=\"%f\""), radius);
	if (bOverride_rigid)
		Out += FString::Printf(TEXT(" rigid=\"%s\""), rigid ? TEXT("true") : TEXT("false"));
	if (bOverride_mass)
		Out += FString::Printf(TEXT(" mass=\"%f\""), mass);
	if (bOverride_inertiabox)
		Out += FString::Printf(TEXT(" inertiabox=\"%f\""), inertiabox);
	...
	if (bOverride_file && !file.IsEmpty())
		Out += FString::Printf(TEXT(" file=\"%s\""), *file);
	...
	if (bOverride_material && !material.IsEmpty())
		Out += FString::Printf(TEXT(" material=\"%s\""), *material);
	if (bOverride_rgba)
		Out += FString::Printf(TEXT(" rgba=\"%f %f %f %f\""), rgba.R, rgba.G, rgba.B, rgba.A);
	if (bOverride_flatskin)
		Out += FString::Printf(TEXT(" flatskin=\"%s\""), flatskin ? TEXT("true") : TEXT("false"));
...
FString UMjFlexcomp::BuildFlexcompXml(const FString& MeshAssetName) const
{
	...
	// UE transform is the authoritative source for placement at runtime.
	{
		...
		if (PosIdx >= 0)
		{
			...
			if (EndIdx > PosIdx)
			{
				Attrs.RemoveAt(PosIdx, EndIdx - PosIdx + 1);
			}
		}
	...
	if (FlexcompType == EMjFlexcompType::Mesh && !MeshAssetName.IsEmpty())
	{
		...
		if (FileIdx >= 0)
		{
			...
			if (EndIdx > FileIdx)
			{
				Attrs.RemoveAt(FileIdx, EndIdx - FileIdx + 1);
			}
		}
	...
	// <contact>
	{
		...
		if (bOverride_ConType)
			ContactAttrs += FString::Printf(TEXT(" contype=\"%d\""), ConType);
		if (bOverride_ConAffinity)
			ContactAttrs += FString::Printf(TEXT(" conaffinity=\"%d\""), ConAffinity);
		if (bOverride_ConDim)
			ContactAttrs += FString::Printf(TEXT(" condim=\"%d\""), ConDim);
		if (bOverride_Priority)
			ContactAttrs += FString::Printf(TEXT(" priority=\"%d\""), Priority);
		if (bOverride_Margin)
			ContactAttrs += FString::Printf(TEXT(" margin=\"%f\""), Margin);
		if (bOverride_Gap)
			ContactAttrs += FString::Printf(TEXT(" gap=\"%f\""), Gap);
		if (bOverride_SelfCollide)
		{
			const TCHAR* SelfStr = TEXT("auto");
			if (SelfCollide == 0)
				SelfStr = TEXT("none");
			else if (SelfCollide == 1)
				SelfStr = TEXT("auto");
			else if (SelfCollide == 2)
				SelfStr = TEXT("all");
	...
	{
		FString EdgeAttrs;
		if (bOverride_EdgeStiffness)
			EdgeAttrs += FString::Printf(TEXT(" stiffness=\"%f\""), EdgeStiffness);
		if (bOverride_EdgeDamping)
			EdgeAttrs += FString::Printf(TEXT(" damping=\"%f\""), EdgeDamping);
	...
	// <elasticity>
	{
		...
		if (bOverride_Young)
			ElasticAttrs += FString::Printf(TEXT(" young=\"%f\""), Young);
		if (bOverride_Poisson)
			ElasticAttrs += FString::Printf(TEXT(" poisson=\"%f\""), Poisson);
		if (bOverride_Damping)
			ElasticAttrs += FString::Printf(TEXT(" damping=\"%f\""), Damping);
		if (bOverride_Thickness)
			ElasticAttrs += FString::Printf(TEXT(" thickness=\"%f\""), Thickness);
...
{
	if (FlexId < 0 || !m_Model || !m_Data || NumRenderVerts == 0)
		return;
	...
	DynamicMesh->EditMesh([&](UE::Geometry::FDynamicMesh3& Mesh) {
		...
		for (int32 i = 0; i + 2 < Indices.Num(); i += 3)
		{
			...
			if (TriId >= 0)
			{
				...
				if (UVOverlay)
					UVOverlay->SetTriangle(TriId, UE::Geometry::FIndex3i(A, B, C));
			}
		}
	},
...
{
	if (!DynamicMesh || FlexId < 0 || !m_Data || NumRenderVerts == 0)
		return;
	...
	Engine->WithRenderState([&](const FMjRenderSnapshot& Snap) {
		...
		if (Snap.FlexvertXPos.Num() < NeededFloats)
			return;

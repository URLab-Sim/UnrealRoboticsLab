...
void UMjGeom::ImportFromXml(const FXmlNode* Node, const FMjCompilerSettings& CompilerSettings)
{
	...
	{ // xml_enum: type -> EMjGeomType
		...
		if (S == TEXT("plane"))
		{
			Type = EMjGeomType::Plane;
			bMatched = true;
		}
		else if (S == TEXT("hfield"))
		{
			Type = EMjGeomType::Hfield;
			bMatched = true;
		}
		else if (S == TEXT("sphere"))
		{
			Type = EMjGeomType::Sphere;
			bMatched = true;
		}
		else if (S == TEXT("capsule"))
		{
	...
	{ // xml_enum: shellinertia -> EMjGeomInertia
		...
		if (S == TEXT("false"))
		{
			ShellInertia = EMjGeomInertia::Volume;
			bMatched = true;
		}
		else if (S == TEXT("true"))
		{
			ShellInertia = EMjGeomInertia::Shell;
			bMatched = true;
		}
		if (bMatched)
			bOverride_ShellInertia = true;
	}
	{ // xml_enum: fluidshape -> EMjFluidShape
		...
		if (S == TEXT("none"))
		{
			FluidShape = EMjFluidShape::None;
			bMatched = true;
		}
		else if (S == TEXT("ellipsoid"))
		{
			FluidShape = EMjFluidShape::Ellipsoid;
			bMatched = true;
		}
		if (bMatched)
			bOverride_FluidShape = true;
	}
	...
	if (MjXmlUtils::ReadAttrString(Node, TEXT("hfield"), hfield))
		bOverride_hfield = true;
	if (MjXmlUtils::ReadAttrString(Node, TEXT("mesh"), mesh))
		bOverride_mesh = true;
...
void UMjGeom::ExportTo(mjsGeom* Element, mjsDefault* Default)
{
	...
	if (bOverride_Type)
	{
		switch (Type)
		{
			case EMjGeomType::Plane:
				FinalType = mjGEOM_PLANE;
				break;
			case EMjGeomType::Hfield:
				FinalType = mjGEOM_HFIELD;
				break;
			case EMjGeomType::Sphere:
				FinalType = mjGEOM_SPHERE;
				break;
			case EMjGeomType::Capsule:
				FinalType = mjGEOM_CAPSULE;
				break;
			case EMjGeomType::Ellipsoid:
				FinalType = mjGEOM_ELLIPSOID;
				break;
			case EMjGeomType::Cylinder:
				FinalType = mjGEOM_CYLINDER;
				break;
			case EMjGeomType::Box:
				FinalType = mjGEOM_BOX;
				break;
			case EMjGeomType::Mesh:
				FinalType = mjGEOM_MESH;
				break;
			case EMjGeomType::SDF:
				FinalType = mjGEOM_SDF;
				break;
		}
	}
	...
	if (bOverride_Type)
	{
		...
		{
			case EMjGeomType::Plane:
				Element->type = (mjtGeom)mjGEOM_PLANE;
				break;
			case EMjGeomType::Hfield:
				Element->type = (mjtGeom)mjGEOM_HFIELD;
				break;
			case EMjGeomType::Sphere:
				Element->type = (mjtGeom)mjGEOM_SPHERE;
				break;
			case EMjGeomType::Capsule:
				Element->type = (mjtGeom)mjGEOM_CAPSULE;
				break;
			case EMjGeomType::Ellipsoid:
				Element->type = (mjtGeom)mjGEOM_ELLIPSOID;
				break;
			case EMjGeomType::Cylinder:
				Element->type = (mjtGeom)mjGEOM_CYLINDER;
				break;
			case EMjGeomType::Box:
				Element->type = (mjtGeom)mjGEOM_BOX;
				break;
			case EMjGeomType::Mesh:
				Element->type = (mjtGeom)mjGEOM_MESH;
				break;
			case EMjGeomType::SDF:
				Element->type = (mjtGeom)mjGEOM_SDF;
				break;
			default:
				break;
		}
	...
	if (bOverride_ShellInertia)
	{
		switch (ShellInertia)
		{
			case EMjGeomInertia::Volume:
				Element->typeinertia = (mjtGeomInertia)mjINERTIA_VOLUME;
				break;
			case EMjGeomInertia::Shell:
				Element->typeinertia = (mjtGeomInertia)mjINERTIA_SHELL;
				break;
			default:
				break;
		}
	...
	if (bOverride_FluidShape)
	{
		switch (FluidShape)
		{
			case EMjFluidShape::None:
				Element->fluid_ellipsoid = (mjtNum)0.0;
				break;
			case EMjFluidShape::Ellipsoid:
				Element->fluid_ellipsoid = (mjtNum)1.0;
				break;
			default:
				break;
		}
	...
	if (bOverride_contype)
		Element->contype = contype;
	if (bOverride_conaffinity)
		Element->conaffinity = conaffinity;
	if (bOverride_condim)
		Element->condim = condim;
	if (bOverride_group)
		Element->group = group;
	...
	if (bOverride_size)
	{
		for (int32 i = 0; i < FMath::Min(size.Num(), 3); ++i)
		{
			if (size[i] != -1.0f)
				Element->size[i] = size[i];
		}
	}
	if (bOverride_friction)
	{
		for (int32 i = 0; i < FMath::Min(friction.Num(), 3); ++i)
			Element->friction[i] = friction[i];
	}
	if (bOverride_mass)
		Element->mass = mass;
	...
	if (bOverride_solmix)
		Element->solmix = solmix;
	if (bOverride_solref)
	{
		for (int32 i = 0; i < FMath::Min(solref.Num(), 2); ++i)
			Element->solref[i] = solref[i];
	}
	if (bOverride_solimp)
	{
		for (int32 i = 0; i < FMath::Min(solimp.Num(), 5); ++i)
			Element->solimp[i] = solimp[i];
	}
	if (bOverride_margin)
	...
	if (bOverride_gap)
		Element->gap = gap;
	if (bOverride_mesh)
		MjSetString(Element->meshname, mesh);
	if (bOverride_fitscale)
		Element->fitscale = fitscale;
	if (bOverride_rgba)
	{
	...
	if (bOverride_fluidcoef)
	{
		for (int32 i = 0; i < FMath::Min(fluidcoef.Num(), 5); ++i)
			Element->fluid_coefs[i] = fluidcoef[i];
	}
	// --- CODEGEN_EXPORT_END ---
}
...
{
	if (friction.Num() == 0)
		friction.Add(NewFriction);
	else
		friction[0] = NewFriction;
...
FString UMjGeom::GetResolvedMaterialName() const
{
	...
	if (UMjDefault* Def = FindEditorDefault())
	{
		...
		while (Cur && !Visited.Contains(Cur->ClassName))
		{
			...
			{
				if (G->bOverride_material && !G->material.IsEmpty())
					return G->material;
			}
...
void UMjGeom::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(UMjGeom, OverrideMaterial) || MemberPropertyName == GET_MEMBER_NAME_CHECKED(UMjGeom, OverrideMaterial))
	{
...
void UMjGeom::RegisterToSpec(FMujocoSpecWrapper& Wrapper, mjsBody* ParentBody)
{
	if (!ParentBody)
		return;
	if (bDisabledByDecomposition)
		return; // Decomposed source geom — hull sub-geoms register instead
	...
	if (!MjClassName.IsEmpty())
	{
		...
		if (!SpecDef)
			SpecDef = mjs_getSpecDefault(Wrapper.Spec);
	}
			...
			for (USceneComponent* Child : Children)
			{
				if (UStaticMeshComponent* SMC = Cast<UStaticMeshComponent>(Child))
				{
					...
					if (AssetNames.Num() > 0)
					{
						...
						for (int32 i = 1; i < AssetNames.Num(); ++i)
							ExtraMeshNames.Add(AssetNames[i]);
						break;
					}
				}
			}

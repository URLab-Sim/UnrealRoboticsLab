...
void UMjJoint::ImportFromXml(const FXmlNode* Node, const FMjCompilerSettings& CompilerSettings)
{
	...
	{ // xml_enum: type -> EMjJointType
		...
		else if (S == TEXT("slide"))
		{
			Type = EMjJointType::Slide;
			bMatched = true;
		}
		else if (S == TEXT("ball"))
		{
	...
	if (bOverride_range)
	{
		...
		else if ((Type == EMjJointType::Hinge) || (Type == EMjJointType::Ball))
		{
			if (!CompilerSettings.bAngleInDegrees)
			{
				for (float& V : range)
				{
					V = FMath::RadiansToDegrees(V);
				}
			}
		}
	}
	if (bOverride_ref)
	{
		...
		else if ((Type == EMjJointType::Hinge) || (Type == EMjJointType::Ball))
		{
			if (!CompilerSettings.bAngleInDegrees)
			{
				ref = FMath::RadiansToDegrees(ref);
			}
		}
	}
	if (bOverride_springref)
	{
		...
		else if ((Type == EMjJointType::Hinge) || (Type == EMjJointType::Ball))
		{
			if (!CompilerSettings.bAngleInDegrees)
			{
				springref = FMath::RadiansToDegrees(springref);
			}
		}
	}
	// --- CODEGEN_IMPORT_END ---
...
void UMjJoint::ExportTo(mjsJoint* Element, mjsDefault* Default)
{
	...
	// axis + pos exports are codegen-emitted between the CODEGEN_EXPORT
	// markers below (spatial_pose canon writes pos, vec3_convert.axis
	// applies the Y-negate to the codegen-emitted axis write).
	...
	if (bOverride_Type)
	{
		switch (Type)
		{
			case EMjJointType::Hinge:
				Element->type = (mjtJoint)mjJNT_HINGE;
				break;
			case EMjJointType::Slide:
				Element->type = (mjtJoint)mjJNT_SLIDE;
				break;
			case EMjJointType::Ball:
				Element->type = (mjtJoint)mjJNT_BALL;
				break;
			case EMjJointType::Free:
				Element->type = (mjtJoint)mjJNT_FREE;
				break;
			default:
				break;
		}
	...
	if (bOverride_springdamper)
	{
		for (int32 i = 0; i < FMath::Min(springdamper.Num(), 2); ++i)
			Element->springdamper[i] = springdamper[i];
	}
	...
	if (bOverride_actuatorgravcomp)
		Element->actgravcomp = actuatorgravcomp;
	if (bOverride_margin)
...
void UMjJoint::Bind(mjModel* Model, mjData* Data, const FString& Prefix)
{
	...
	BindAndCacheView(m_JointView, Prefix);
}
...
{
	if (m_JointView.id < 0 || !m_JointView.jnt_range)
		return FVector2D::ZeroVector;
	return FVector2D((float)m_JointView.jnt_range[0], (float)m_JointView.jnt_range[1]);
}
...
{
	if (m_JointView.id < 0 || !m_JointView.name)
		return FString();
...
FMuJoCoJointState UMjJoint::GetJointState() const
{
	...
	{
		if (m_JointView.qpos)
			State.Position = (float)m_JointView.qpos[0];
		if (m_JointView.qvel)
			State.Velocity = (float)m_JointView.qvel[0];
		if (m_JointView.qacc)
			State.Acceleration = (float)m_JointView.qacc[0];
	}
	return State;
}
...
EMjJointType UMjJoint::GetResolvedType() const
{
	...
	if (m_JointView.id >= 0)
	{
		switch (m_JointView.jnt_type)
		{
			case mjJNT_FREE:
				return EMjJointType::Free;
			case mjJNT_BALL:
				return EMjJointType::Ball;
			case mjJNT_SLIDE:
				return EMjJointType::Slide;
			case mjJNT_HINGE:
				return EMjJointType::Hinge;
			default:
				return EMjJointType::Hinge;
		}
	}
...
FVector2D UMjJoint::GetResolvedRange() const
{
	...
	auto MjToUE = [T](float V) -> float {
		if (T == EMjJointType::Slide)
			return V * 100.0f; // m -> cm
	...
	if (m_JointView.id >= 0 && m_JointView.jnt_range)
	{
		return FVector2D(MjToUE((float)m_JointView.jnt_range[0]),
			MjToUE((float)m_JointView.jnt_range[1]));
	}
...
bool UMjJoint::GetResolvedLimited() const
{
	...
	if (m_JointView.id >= 0 && m_JointView.jnt_range)
	{
		...
		return m_JointView.jnt_range[0] != 0.0 || m_JointView.jnt_range[1] != 0.0;
	}

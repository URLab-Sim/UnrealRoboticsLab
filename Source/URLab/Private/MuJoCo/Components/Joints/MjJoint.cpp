...
void UMjJoint::ImportFromXml(const FXmlNode* Node, const FMjCompilerSettings& CompilerSettings)
{
    ...
    if (bOverride_Axis) Axis.Y = -Axis.Y;
...
void UMjJoint::ExportTo(mjsJoint* Element, mjsDefault* Default)
{
    ...
    if (bOverride_Axis) { Element->axis[0] = Axis.X; Element->axis[1] = -Axis.Y; Element->axis[2] = Axis.Z; }
    if (bOverride_springdamper) { for (int32 i = 0; i < FMath::Min(springdamper.Num(), 2); ++i) Element->springdamper[i] = springdamper[i]; }
...
{
    if (m_JointView.id < 0 || !m_JointView.jnt_range) return FVector2D::ZeroVector;

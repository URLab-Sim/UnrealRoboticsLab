...
void UMjKeyframe::ExportTo(mjsKey* Element, mjsDefault* Default)
{
	...
	if (bOverride_Time)
		Element->time = Time;
	if (bOverride_Qpos)
		MjSetDoubleVec(Element->qpos, Qpos);
	if (bOverride_Qvel)
		MjSetDoubleVec(Element->qvel, Qvel);
	if (bOverride_Act)
		MjSetDoubleVec(Element->act, Act);
	if (bOverride_Ctrl)
		MjSetDoubleVec(Element->ctrl, Ctrl);
	if (bOverride_Mpos)
		MjSetDoubleVec(Element->mpos, Mpos);
	if (bOverride_Mquat)
		MjSetDoubleVec(Element->mquat, Mquat);
	// --- CODEGEN_EXPORT_END ---
}

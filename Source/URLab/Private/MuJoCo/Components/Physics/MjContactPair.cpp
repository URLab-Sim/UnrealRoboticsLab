...
void UMjContactPair::ImportFromXml(const FXmlNode* Node, const FMjCompilerSettings& CompilerSettings)
{
	...
	if (MjXmlUtils::ReadAttrString(Node, TEXT("geom1"), geom1))
		bOverride_geom1 = true;
	if (MjXmlUtils::ReadAttrString(Node, TEXT("geom2"), geom2))
		bOverride_geom2 = true;
...
void UMjContactPair::ExportTo(mjsPair* Element)
{
	...
	if (bOverride_geom1)
		MjSetString(Element->geomname1, geom1);
	if (bOverride_geom2)
		MjSetString(Element->geomname2, geom2);
	if (bOverride_condim)
		Element->condim = condim;
	if (bOverride_friction)
	{
		for (int32 i = 0; i < FMath::Min(friction.Num(), 5); ++i)
	...
	if (bOverride_solreffriction)
	{
		for (int32 i = 0; i < FMath::Min(solreffriction.Num(), 2); ++i)
			Element->solreffriction[i] = solreffriction[i];
	}
	if (bOverride_solimp)
	{

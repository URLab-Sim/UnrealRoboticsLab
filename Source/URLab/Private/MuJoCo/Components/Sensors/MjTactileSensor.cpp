...
{
    Type = EMjSensorType::Tactile;
}

void UMjTactileSensor::Bind(mjModel* Model, mjData* Data, const FString& Prefix)
{
    Super::Bind(Model, Data, Prefix);
    // Sync Dim from the compiled model so Blueprint inspection shows the real count.
    // Tactile sensor dimension is set by the MuJoCo compiler based on the contact

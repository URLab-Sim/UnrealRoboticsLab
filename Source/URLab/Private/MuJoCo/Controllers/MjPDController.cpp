...
void UMjPDController::Bind(mjModel* m, mjData* d, const TMap<int32, UMjNodeComponent*>& ActuatorIdMap)
{
...
FString UMjPDController::GetBindingLocalJointName(int32 Index) const
{
	if (!Bindings.IsValidIndex(Index) || !Bindings[Index].Component)
		return FString();
	const UMjNodeComponent& Element = *Bindings[Index].Component;
	const FString FullName = Element.MjName.Get(Element.GetName());

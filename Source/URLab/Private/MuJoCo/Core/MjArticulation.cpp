...
void AMjArticulation::Setup(mjSpec* Spec, mjVFS* VFS)
{
	...
	if (ChildWorld)
	{
		for (USceneComponent* Child : WorldChildren)
		{
			if (Cast<UMjBody>(Child) || Cast<UMjFrame>(Child))
				continue;
...
void AMjArticulation::PostSetup(mjModel* Model, mjData* Data)
{
	...
	{
		if (MjComp)
			MjComp->Bind(Model, Data, m_prefix);
	}
	...
	for (UMjComponent* MjComp : AllMjComponents)
	{
		...
		auto AddToMap = [&](auto& Map, auto* Comp) {
			if (!UE_Name.IsEmpty())
				Map.Add(UE_Name, Comp);
			if (!MJ_Name.IsEmpty() && MJ_Name != UE_Name)
				Map.Add(MJ_Name, Comp);
		};

		if (UMjActuator* A = Cast<UMjActuator>(MjComp))
			AddToMap(ActuatorComponentMap, A);
		else if (UMjJoint* J = Cast<UMjJoint>(MjComp))
			AddToMap(JointComponentMap, J);
		else if (UMjSensor* S = Cast<UMjSensor>(MjComp))
			AddToMap(SensorComponentMap, S);
		else if (UMjBody* B = Cast<UMjBody>(MjComp))
			AddToMap(BodyComponentMap, B);
		else if (UMjTendon* T = Cast<UMjTendon>(MjComp))
			AddToMap(TendonComponentMap, T);
		else if (UMjEquality* E = Cast<UMjEquality>(MjComp))
			AddToMap(EqualityComponentMap, E);
		else if (UMjKeyframe* K = Cast<UMjKeyframe>(MjComp))
			AddToMap(KeyframeComponentMap, K);
	}
	...
	for (UMjComponent* MjComp : AllMjComponents)
	{
		...
		if (UMjBody* Body = Cast<UMjBody>(MjComp))
			BodyIdMap.Add(ID, Body);
		else if (UMjGeom* Geom = Cast<UMjGeom>(MjComp))
			GeomIdMap.Add(ID, Geom);
		else if (UMjJoint* Joint = Cast<UMjJoint>(MjComp))
			JointIdMap.Add(ID, Joint);
		else if (UMjSensor* Sensor = Cast<UMjSensor>(MjComp))
			SensorIdMap.Add(ID, Sensor);
		else if (UMjActuator* Actuator = Cast<UMjActuator>(MjComp))
			ActuatorIdMap.Add(ID, Actuator);
		else if (UMjTendon* Tendon = Cast<UMjTendon>(MjComp))
			TendonIdMap.Add(ID, Tendon);
	}
...
TArray<UMjEquality*> AMjArticulation::GetEqualities() const
{
	...
	for (UMjEquality* E : Components)
		UniqueComponents.AddUnique(E);
...
TArray<UMjKeyframe*> AMjArticulation::GetKeyframes() const
{
	...
	for (UMjKeyframe* K : Components)
		UniqueComponents.AddUnique(K);
	return UniqueComponents;
}
...
TArray<FString> AMjArticulation::GetKeyframeNames() const
{
	...
	{
		if (K)
			Names.Add(K->MjName.IsEmpty() ? K->GetName() : K->MjName);
	}
	return Names;
}
...
UMjComponent* AMjArticulation::GetComponentByMjId(mjtObj type, int32 id) const
{
	...
	{
		case mjOBJ_BODY:
			return GetComponentByIdInMap(id, BodyIdMap);
		case mjOBJ_GEOM:
			return GetComponentByIdInMap(id, GeomIdMap);
		case mjOBJ_JOINT:
			return GetComponentByIdInMap(id, JointIdMap);
		case mjOBJ_SENSOR:
			return GetComponentByIdInMap(id, SensorIdMap);
		case mjOBJ_ACTUATOR:
			return GetComponentByIdInMap(id, ActuatorIdMap);
		default:
			return nullptr;
	}
}
	...
	for (UMjSite* Site : Sites)
	{
		...
		if (Site->IsBound())
		{
			...
			Color = (SV.site_rgba) ? FColor(
										 ...
										 (uint8)(SV.site_rgba[2] * 255), 200)
								   : FColor(128, 128, 128, 200);
		}
		else
		{

...
void UMjDebugVisualizer::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	{
		float force = 0.0f;
		if (i < LocalDebugData.ContactForces.Num())
			force = LocalDebugData.ContactForces[i];
...
void UMjDebugVisualizer::CaptureDebugData()
{
	...
	for (int32 b = 0; b < Model->nbody; ++b)
	{
		...
		if (WeldId < 0 || WeldId >= Model->nbody)
			continue;
		if (!Model->body_dofnum || Model->body_dofnum[WeldId] == 0)
			continue;
		...
		if (H == -1 && bSleepEnabled && Model->dof_treeid && Model->tree_dofadr)
		{
			...
			if (!bBodyAwake && Data->tree_asleep && Model->ntree > 0 && Tree >= 0 && Tree < Model->ntree)
			{
	...
	{
		for (int32 i = 0; i < NumWrapObj; ++i)
			DebugData.WrapObj[i] = Data->wrap_obj[i];
	}
	...
	{
		if (Model->actuator_trntype[a] != mjTRN_TENDON)
			continue;
		if (Model->actuator_dyntype[a] != mjDYN_MUSCLE)
			continue;
		const int32 TargetTendon = Model->actuator_trnid[a * 2];
		const int32 ActAdr = Model->actuator_actadr ? Model->actuator_actadr[a] : -1;
		if (TargetTendon < 0 || TargetTendon >= Model->ntendon)
			continue;
		if (ActAdr < 0 || !Data->act)
			continue;
...
void UMjDebugVisualizer::CycleDebugShaderMode()
{
	...
	switch (DebugShaderMode)
	{
		...
		case EMjDebugShaderMode::InstanceSegmentation:
			Label = TEXT("Instance Segmentation");
			break;
		case EMjDebugShaderMode::SemanticSegmentation:
			Label = TEXT("Semantic Segmentation");
			break;
		default:
			break;
	}
...
void UMjDebugVisualizer::GetOverlaySnapshot(TArray<int32>& OutBodyAwake, TArray<int32>& OutBodyIslandSeed)
{
	FScopeLock Lock(&DebugMutex);
	OutBodyAwake = DebugData.BodyAwake;
	OutBodyIslandSeed = DebugData.BodyIslandSeed;
}
...
void UMjDebugVisualizer::UpdateTendonTubes()
{
	...
	for (int32 t = 0; t < NumTendons; ++t)
	{
		...
		{
			if (LocalWrapObj.IsValidIndex(j) && LocalWrapObj[j] == -2)
				continue;
			if (LocalWrapObj.IsValidIndex(j + 1) && LocalWrapObj[j + 1] == -2)
				continue;
			if (!LocalPoints.IsValidIndex(j + 1))
				continue;

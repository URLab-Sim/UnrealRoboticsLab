...
void AMjImpulseLauncher::ResetAndFire()
{
	...
	int BodyId = ResolvedBody->GetBoundId().Get(-1);
	if (BodyId < 0)
		return;
	...
	for (int j = 0; j < m->njnt; ++j)
	{
		if (m->jnt_bodyid[j] == BodyId && m->jnt_type[j] == mjJNT_FREE)
		{
			...
			mjtNum mjPos[3];
			URLabAxisConv::UePositionToMj(LaunchPos, mjPos);
...
void AMjImpulseLauncher::FireImpulse()
{
	...
	if (LaunchTarget)
	{
		...
		if (TargetMjBody != nullptr && TargetMjBody->GetBoundId().IsSet())
		{
			EndPos = TargetMjBody->GetWorldPosition();
		}
		else
		{
	...
	mjtNum mjVel[3];
	URLabAxisConv::UePositionToMj(LaunchVelocityUE, mjVel);

		...
		[this](mjModel* m, mjData* d) {
			...
			if (bPuppet)
			{
				...
				if (Perturb.select > 0 && Perturb.active != 0)
				{
					...
					if (Scratch.Num() < 6 * m->nbody)
						Scratch.SetNum(6 * m->nbody);
...
void UMjPerturbation::StartTranslate(const FVector& CursorOrigin, const FVector& CursorDirection)
{
	if (!HasSelection() || !Manager || !Manager->PhysicsEngine)
		return;
	if (!Manager->PhysicsEngine->m_model || !Manager->PhysicsEngine->m_data)
		return;
...
{
	if (!HasSelection() || !IsDragging() || !Manager || !Manager->PhysicsEngine)
		return;

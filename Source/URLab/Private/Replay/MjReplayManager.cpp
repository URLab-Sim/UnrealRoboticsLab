...
bool AMjReplayManager::LoadFromCSV(FString FilePath, float Timestep)
{
	...
	{
		if (Headers[i].Equals(TEXT("time"), ESearchCase::IgnoreCase) || Headers[i].Equals(TEXT("timestamp"), ESearchCase::IgnoreCase))
		{
			TimeCol = i;
			break;
		}
	...
	for (int32 i = 0; i < Headers.Num(); ++i)
	{
		...
		// Camera columns
		if (ColName.Equals(TEXT("cam_x"), ESearchCase::IgnoreCase))
		{
			CamXCol = i;
			continue;
		}
		if (ColName.Equals(TEXT("cam_y"), ESearchCase::IgnoreCase))
		{
			CamYCol = i;
			continue;
		}
		if (ColName.Equals(TEXT("cam_z"), ESearchCase::IgnoreCase))
		{
			CamZCol = i;
			continue;
		}
		if (ColName.Equals(TEXT("cam_pitch"), ESearchCase::IgnoreCase))
		{
			CamPitchCol = i;
			continue;
		}
		if (ColName.Equals(TEXT("cam_yaw"), ESearchCase::IgnoreCase))
		{
			CamYawCol = i;
			continue;
		}
		if (ColName.Equals(TEXT("cam_roll"), ESearchCase::IgnoreCase))
		{
			CamRollCol = i;
			continue;
		}
	...
	for (int32 LineIdx = 1; LineIdx < Lines.Num(); ++LineIdx)
	{
		...
		if (bHasCameraColumns)
		{
			...
			if (CamXCol < Values.Num())
				Frame.CameraPosition.X = FCString::Atod(*Values[CamXCol]);
			if (CamYCol < Values.Num())
				Frame.CameraPosition.Y = FCString::Atod(*Values[CamYCol]);
			if (CamZCol < Values.Num())
				Frame.CameraPosition.Z = FCString::Atod(*Values[CamZCol]);
			if (CamPitchCol < Values.Num())
				Frame.CameraRotation.Pitch = FCString::Atod(*Values[CamPitchCol]);
			if (CamYawCol < Values.Num())
				Frame.CameraRotation.Yaw = FCString::Atod(*Values[CamYawCol]);
			if (CamRollCol < Values.Num())
				Frame.CameraRotation.Roll = FCString::Atod(*Values[CamRollCol]);
		}
...
{
	if (!bIsRecording || !m || !d || !d->qpos || !d->qvel)
		return;
	...
	for (int i = 0; i < m->njnt; ++i)
	{
		...
		if (qpos_dim > 0)
		{
			...
			for (int k = 0; k < qpos_dim; ++k)
				Kinematics.QPos[k] = d->qpos[qpos_adr + k];
			for (int k = 0; k < qvel_dim; ++k)
				Kinematics.QVel[k] = d->qvel[qvel_adr + k];
		...
		if (Kinematics)
		{
			...
			if (Kinematics->QPos.Num() == expected_dim)
			{
				...
				if (type == mjJNT_FREE && Binding && Binding->bRelativePosition && expected_dim == 7)
				{
					...
					{
						for (int k = 3; k < 7; ++k)
							d->qpos[qpos_adr + k] = Kinematics->QPos[k];
					}
				}
				else
				{
					for (int k = 0; k < expected_dim; ++k)
						d->qpos[qpos_adr + k] = LerpQPos(k);
				}
			}
			...
			{
				for (int k = 0; k < expected_vel_dim; ++k)
					d->qvel[qvel_adr + k] = Kinematics->QVel[k];
			}
		}
		else
		{

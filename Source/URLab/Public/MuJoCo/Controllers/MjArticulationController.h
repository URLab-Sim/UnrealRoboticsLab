...
struct FActuatorBinding
{
	...
	/** The actuator element this binding drives. */
	UPROPERTY()
	TObjectPtr<UMjNodeComponent> Component = nullptr;
};
 ...
 *         {
 *             float target = UMjActuatorRuntime::GetControl(Bindings[i].Component);
...
public:
	 ...
	 */
	virtual void Bind(mjModel* m, mjData* d, const TMap<int32, UMjNodeComponent*>& ActuatorIdMap);

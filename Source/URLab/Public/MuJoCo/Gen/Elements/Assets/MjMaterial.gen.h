// Copyright (c) 2026 Jonathan Embley-Riches. All rights reserved.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

// Generated by protospec_gen.emit_ue -- do not edit.
//
// <material>: 11 schema attributes, of which 10 carry storage here
// (the identity attribute lives on UMjNodeComponent::MjName).
#pragma once

#include "CoreMinimal.h"
#include "MuJoCo/Spec/MjNodeComponent.h"

#include "MjMaterial.gen.generated.h"

UCLASS(ClassGroup = (MuJoCo), meta = (BlueprintSpawnableComponent))
class URLAB_API UMjMaterial : public UMjNodeComponent
{
	GENERATED_BODY()

public:
	/** Defaults class for setting unspecified attributes. (MJCF: class) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "Defaults class for setting unspecified attributes. (MJCF: class)", GetOptions = "GetDclassOptions"))
	TOptional<FString> Dclass;

	/** This attribute applies to textures of type '2d'. It specifies how many times the texture image is repeated, relative to either the object size or the spatial unit, as determined by the next attribute. (MJCF: texrepeat) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "This attribute applies to textures of type '2d'. It specifies how many times the texture image is repeated, relative to either the object size or the spatial unit, as determined by the next attribute. (MJCF: texrepeat)"))
	TOptional<TArray<float>> Texrepeat;

	/** For cube textures, this attribute controls how cube mapping is applied. The default value 'false' means apply cube mapping directly, using the actual size of the object. The value 'true' maps the texture to a unit object before scaling it to its actual size (geometric primitives are created by the renderer as unit objects and then scaled). (MJCF: texuniform) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "For cube textures, this attribute controls how cube mapping is applied. The default value 'false' means apply cube mapping directly, using the actual size of the object. The value 'true' maps the texture to a unit object before scaling it to its actual size (geometric primitives are created by the renderer as unit objects and then scaled). (MJCF: texuniform)"))
	TOptional<bool> Texuniform;

	/** Emission in OpenGL has the RGBA format, however we only provide a scalar setting. The RGB components of the OpenGL emission vector are the RGB components of the material color multiplied by the value specified here. The alpha component is 1. (MJCF: emission) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "Emission in OpenGL has the RGBA format, however we only provide a scalar setting. The RGB components of the OpenGL emission vector are the RGB components of the material color multiplied by the value specified here. The alpha component is 1. (MJCF: emission)"))
	TOptional<float> Emission;

	/** Specularity in OpenGL has the RGBA format, however we only provide a scalar setting. The RGB components of the OpenGL specularity vector are all equal to the value specified here. The alpha component is 1. This value should be in the range [0 1]. (MJCF: specular) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "Specularity in OpenGL has the RGBA format, however we only provide a scalar setting. The RGB components of the OpenGL specularity vector are all equal to the value specified here. The alpha component is 1. This value should be in the range [0 1]. (MJCF: specular)"))
	TOptional<float> Specular;

	/** Shininess in OpenGL is a number between 0 and 128. The value given here is multiplied by 128 before passing it to OpenGL, so it should be in the range [0 1]. Larger values correspond to tighter specular highlight (thus reducing the overall amount of highlight but making it more salient visually). This interacts with the specularity setting; see OpenGL documentation for details. (MJCF: shininess) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "Shininess in OpenGL is a number between 0 and 128. The value given here is multiplied by 128 before passing it to OpenGL, so it should be in the range [0 1]. Larger values correspond to tighter specular highlight (thus reducing the overall amount of highlight but making it more salient visually). This interacts with the specularity setting; see OpenGL documentation for details. (MJCF: shininess)"))
	TOptional<float> Shininess;

	/** This attribute should be in the range [0 1]. If the value is greater than 0, and the material is applied to a plane or a box geom, the renderer will simulate reflectance. The larger the value, the stronger the reflectance. For boxes, only the face in the direction of the local +Z axis is reflective. Simulating reflectance properly requires ray-tracing. (MJCF: reflectance) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "This attribute should be in the range [0 1]. If the value is greater than 0, and the material is applied to a plane or a box geom, the renderer will simulate reflectance. The larger the value, the stronger the reflectance. For boxes, only the face in the direction of the local +Z axis is reflective. Simulating reflectance properly requires ray-tracing. (MJCF: reflectance)"))
	TOptional<float> Reflectance;

	/** This attribute corresponds to uniform metallicity coefficient applied to the entire material. This attribute has no effect in MuJoCo's native renderer, but it can be useful when rendering scenes with a physically-based renderer. (MJCF: metallic) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "This attribute corresponds to uniform metallicity coefficient applied to the entire material. This attribute has no effect in MuJoCo's native renderer, but it can be useful when rendering scenes with a physically-based renderer. (MJCF: metallic)"))
	TOptional<float> Metallic;

	/** This attribute corresponds to uniform roughness coefficient applied to the entire material. This attribute has no effect in MuJoCo's native renderer, but it can be useful when rendering scenes with a physically-based renderer. (MJCF: roughness) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "This attribute corresponds to uniform roughness coefficient applied to the entire material. This attribute has no effect in MuJoCo's native renderer, but it can be useful when rendering scenes with a physically-based renderer. (MJCF: roughness)"))
	TOptional<float> Roughness;

	/** Color and transparency of the material. All components should be in the range [0 1]. Note that the texture color (if assigned) and the color specified here are multiplied component-wise. Thus the default value of '1 1 1 1' has the effect of leaving the texture unchanged. (MJCF: rgba) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "Color and transparency of the material. All components should be in the range [0 1]. Note that the texture color (if assigned) and the color specified here are multiplied component-wise. Thus the default value of '1 1 1 1' has the effect of leaving the texture unchanged. (MJCF: rgba)"))
	TOptional<FLinearColor> Rgba;

	// --- Blueprint access ---
	// TOptional has no Blueprint pin, so presence and value reach
	// Blueprint through this quartet. Get() falls back to the schema
	// default; it does NOT resolve the default-class chain, which is
	// EffectiveField's job.
	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasDclass() const { return Dclass.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	FString GetDclass() const { return Dclass.Get(FString()); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetDclass(const FString& InValue) { Dclass = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearDclass() { Dclass.Reset(); }

	UFUNCTION()
	TArray<FString> GetDclassOptions() const
	{ return RefNameOptions(1); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasTexrepeat() const { return Texrepeat.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	TArray<float> GetTexrepeat() const { return Texrepeat.Get(TArray<float>({1.0f, 1.0f})); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetTexrepeat(const TArray<float>& InValue) { Texrepeat = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearTexrepeat() { Texrepeat.Reset(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasTexuniform() const { return Texuniform.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool GetTexuniform() const { return Texuniform.Get(false); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetTexuniform(bool InValue) { Texuniform = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearTexuniform() { Texuniform.Reset(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasEmission() const { return Emission.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	float GetEmission() const { return Emission.Get(0.0f); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetEmission(float InValue) { Emission = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearEmission() { Emission.Reset(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasSpecular() const { return Specular.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	float GetSpecular() const { return Specular.Get(0.5f); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetSpecular(float InValue) { Specular = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearSpecular() { Specular.Reset(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasShininess() const { return Shininess.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	float GetShininess() const { return Shininess.Get(0.5f); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetShininess(float InValue) { Shininess = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearShininess() { Shininess.Reset(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasReflectance() const { return Reflectance.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	float GetReflectance() const { return Reflectance.Get(0.0f); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetReflectance(float InValue) { Reflectance = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearReflectance() { Reflectance.Reset(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasMetallic() const { return Metallic.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	float GetMetallic() const { return Metallic.Get(-1.0f); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetMetallic(float InValue) { Metallic = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearMetallic() { Metallic.Reset(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasRoughness() const { return Roughness.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	float GetRoughness() const { return Roughness.Get(-1.0f); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetRoughness(float InValue) { Roughness = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearRoughness() { Roughness.Reset(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasRgba() const { return Rgba.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	FLinearColor GetRgba() const { return Rgba.Get(FLinearColor(1.0f, 1.0f, 1.0f, 1.0f)); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetRgba(FLinearColor InValue) { Rgba = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearRgba() { Rgba.Reset(); }

};

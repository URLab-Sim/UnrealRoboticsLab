// Copyright (c) 2026 Jonathan Embley-Riches. All rights reserved.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

// Generated by protospec_gen.emit_ue -- do not edit.
//
// <flex>: 15 schema attributes, of which 14 carry storage here
// (the identity attribute lives on UMjNodeComponent::MjName).
#pragma once

#include "CoreMinimal.h"
#include "MuJoCo/Spec/MjNodeComponent.h"
#include "MuJoCo/Gen/MjEnums.gen.h"

#include "MjFlex.gen.generated.h"

UCLASS(ClassGroup = (MuJoCo), meta = (BlueprintSpawnableComponent))
class URLAB_API UMjFlex : public UMjNodeComponent
{
	GENERATED_BODY()

public:
	/** Integer group to which the flex belongs. This attribute can be used for custom tags. It is also used by the visualizer to enable and disable the rendering of entire groups of flexes. (MJCF: group) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "Integer group to which the flex belongs. This attribute can be used for custom tags. It is also used by the visualizer to enable and disable the rendering of entire groups of flexes. (MJCF: group)"))
	TOptional<int32> Group;

	/** Dimensionality of the flex. Allowed values are 1, 2 and 3. In 1D the elements are capsules, in 2D the elements are triangles with radius, in 3D the elements are tetrahedra with (optional) radius. (MJCF: dim) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "Dimensionality of the flex. Allowed values are 1, 2 and 3. In 1D the elements are capsules, in 2D the elements are triangles with radius, in 3D the elements are tetrahedra with (optional) radius. (MJCF: dim)"))
	TOptional<int32> Dim;

	/** Radius of all flex elements. It can be zero in 3D, but must be positive in 1D and 2D. The radius affects both collision detection and rendering. In 1D and 2D it is needed to make the elements volumetric. (MJCF: radius) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "Radius of all flex elements. It can be zero in 3D, but must be positive in 1D and 2D. The radius affects both collision detection and rendering. In 1D and 2D it is needed to make the elements volumetric. (MJCF: radius)"))
	TOptional<double> Radius;

	/** If specified, this attribute applies a material to the flex. Note that textures specified in the material will be applied only if the flex has explicit texture coordinates. (MJCF: material) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "If specified, this attribute applies a material to the flex. Note that textures specified in the material will be applied only if the flex has explicit texture coordinates. (MJCF: material)", GetOptions = "GetMaterialOptions"))
	TOptional<FString> Material;

	/** Instead of creating material assets and referencing them, this attribute can be used to set color and transparency only. This is not as flexible as the material mechanism, but is more convenient and is often sufficient. If the value of this attribute is different from the internal default, it takes precedence over the material. (MJCF: rgba) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "Instead of creating material assets and referencing them, this attribute can be used to set color and transparency only. This is not as flexible as the material mechanism, but is more convenient and is often sufficient. If the value of this attribute is different from the internal default, it takes precedence over the material. (MJCF: rgba)"))
	TOptional<FLinearColor> Rgba;

	/** This attribute determines whether 2D and 3D flexes that are rendered in flexskin mode will use smooth or flat shading. The default smooth shading is suitable in most cases, however if the object is intended to have visible sharp edges (such as a cube) then flat shading is more natural. (MJCF: flatskin) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "This attribute determines whether 2D and 3D flexes that are rendered in flexskin mode will use smooth or flat shading. The default smooth shading is suitable in most cases, however if the object is intended to have visible sharp edges (such as a cube) then flat shading is more natural. (MJCF: flatskin)"))
	TOptional<bool> Flatskin;

	/** An array of MuJoCo body names (separated by white space) to which each vertex belongs. The number of body names should either equal the number of vertices (nvert), or be a single body. If a single body is specified, all vertices are defined within that body - in which case the flex becomes a rigid body. (MJCF: body) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "An array of MuJoCo body names (separated by white space) to which each vertex belongs. The number of body names should either equal the number of vertices (nvert), or be a single body. If a single body is specified, all vertices are defined within that body - in which case the flex becomes a rigid body. (MJCF: body)"))
	FString Body;

	/** The local coordinates of the vertices within the corresponding body frames. If this attribute is omitted, all coordinates are (0,0,0) or in other words, the vertices coincide with the centers of the body frames. (MJCF: vertex) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "The local coordinates of the vertices within the corresponding body frames. If this attribute is omitted, all coordinates are (0,0,0) or in other words, the vertices coincide with the centers of the body frames. (MJCF: vertex)"))
	TOptional<TArray<double>> Vertex;

	/** For each element of the flex, this lists the zero-based indices of the vertices forming that flex element. We need two vertices to specify a capsule, three vertices to specify a triangle, and four vertices to specify a tetrahedron - which is why the number of indices equals (dim+1) times the number of elements. In 2D, the vertices should be listed in counter-clockwise order. (MJCF: element) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "For each element of the flex, this lists the zero-based indices of the vertices forming that flex element. We need two vertices to specify a capsule, three vertices to specify a triangle, and four vertices to specify a tetrahedron - which is why the number of indices equals (dim+1) times the number of elements. In 2D, the vertices should be listed in counter-clockwise order. (MJCF: element)"))
	TArray<int32> Element;

	/** Texture coordinates. If omitted, texture mapping for this flex is disabled, even if a texture is specified in the material. (MJCF: texcoord) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "Texture coordinates. If omitted, texture mapping for this flex is disabled, even if a texture is specified in the material. (MJCF: texcoord)"))
	TOptional<TArray<float>> Texcoord;

	/** Texture indices for each face. If omitted, texture are assumed to be vertex-based. (MJCF: elemtexcoord) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "Texture indices for each face. If omitted, texture are assumed to be vertex-based. (MJCF: elemtexcoord)"))
	TOptional<TArray<int32>> Elemtexcoord;

	/** The degrees-of-freedom of the flex. An array of MuJoCo body names (separated by white space) to which each node belongs. The number of body names should equal the number of nodes (nnode). See the flexcomp dof attribute for more details. (MJCF: node) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "The degrees-of-freedom of the flex. An array of MuJoCo body names (separated by white space) to which each node belongs. The number of body names should equal the number of nodes (nnode). See the flexcomp dof attribute for more details. (MJCF: node)"))
	TOptional<FString> Node;

	/** When using trilinear or quadratic dofs, this specifies the number of cells in each dimension for the background interpolation grid. (MJCF: cellcount) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "When using trilinear or quadratic dofs, this specifies the number of cells in each dimension for the background interpolation grid. (MJCF: cellcount)"))
	TOptional<TArray<int32>> Cellcount;

	/** Interpolation order for the flex. (MJCF: dof) */
	UPROPERTY(EditAnywhere, Category = "MuJoCo", meta = (ToolTip = "Interpolation order for the flex. (MJCF: dof)"))
	TOptional<EMjFlexDof> Dof;

	// --- Blueprint access ---
	// TOptional has no Blueprint pin, so presence and value reach
	// Blueprint through this quartet. Get() falls back to the schema
	// default; it does NOT resolve the default-class chain, which is
	// EffectiveField's job.
	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasGroup() const { return Group.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	int32 GetGroup() const { return Group.Get(0); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetGroup(int32 InValue) { Group = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearGroup() { Group.Reset(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasDim() const { return Dim.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	int32 GetDim() const { return Dim.Get(2); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetDim(int32 InValue) { Dim = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearDim() { Dim.Reset(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasRadius() const { return Radius.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	double GetRadius() const { return Radius.Get(0.005); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetRadius(double InValue) { Radius = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearRadius() { Radius.Reset(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasMaterial() const { return Material.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	FString GetMaterial() const { return Material.Get(FString()); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetMaterial(const FString& InValue) { Material = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearMaterial() { Material.Reset(); }

	UFUNCTION()
	TArray<FString> GetMaterialOptions() const
	{ return RefNameOptions(4); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasRgba() const { return Rgba.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	FLinearColor GetRgba() const { return Rgba.Get(FLinearColor(0.5f, 0.5f, 0.5f, 1.0f)); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetRgba(FLinearColor InValue) { Rgba = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearRgba() { Rgba.Reset(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasFlatskin() const { return Flatskin.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool GetFlatskin() const { return Flatskin.Get(false); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetFlatskin(bool InValue) { Flatskin = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearFlatskin() { Flatskin.Reset(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasBody() const { return true; }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	FString GetBody() const { return Body; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetBody(const FString& InValue) { Body = InValue; }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasVertex() const { return Vertex.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	TArray<double> GetVertex() const { return Vertex.Get(TArray<double>()); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetVertex(const TArray<double>& InValue) { Vertex = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearVertex() { Vertex.Reset(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasElement() const { return true; }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	TArray<int32> GetElement() const { return Element; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetElement(const TArray<int32>& InValue) { Element = InValue; }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasTexcoord() const { return Texcoord.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	TArray<float> GetTexcoord() const { return Texcoord.Get(TArray<float>()); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetTexcoord(const TArray<float>& InValue) { Texcoord = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearTexcoord() { Texcoord.Reset(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasElemtexcoord() const { return Elemtexcoord.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	TArray<int32> GetElemtexcoord() const { return Elemtexcoord.Get(TArray<int32>()); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetElemtexcoord(const TArray<int32>& InValue) { Elemtexcoord = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearElemtexcoord() { Elemtexcoord.Reset(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasNode() const { return Node.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	FString GetNode() const { return Node.Get(FString()); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetNode(const FString& InValue) { Node = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearNode() { Node.Reset(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasCellcount() const { return Cellcount.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	TArray<int32> GetCellcount() const { return Cellcount.Get(TArray<int32>({1, 1, 1})); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetCellcount(const TArray<int32>& InValue) { Cellcount = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearCellcount() { Cellcount.Reset(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	bool HasDof() const { return Dof.IsSet(); }

	UFUNCTION(BlueprintPure, Category = "MuJoCo")
	EMjFlexDof GetDof() const { return Dof.Get(static_cast<EMjFlexDof>(0)); }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void SetDof(EMjFlexDof InValue) { Dof = InValue; }

	UFUNCTION(BlueprintCallable, Category = "MuJoCo")
	void ClearDof() { Dof.Reset(); }

};

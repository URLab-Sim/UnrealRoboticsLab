...
namespace URLabAxisConv
{
    ...
    FORCEINLINE FQuat MjQuatToUe(const double InWXYZ[4])
    {
        ...
        return FQuat(
            ...
            static_cast<FQuat::FReal>( InWXYZ[0])).GetNormalized();
    }
    ...
    FORCEINLINE void YNegate(FVector& InOut) { InOut.Y = -InOut.Y; }
    ...
    FORCEINLINE double ScaleMToCm(double InM) { return InM * 100.0; }
    FORCEINLINE double ScaleCmToM(double InCm) { return InCm * 0.01; }
} // namespace URLabAxisConv

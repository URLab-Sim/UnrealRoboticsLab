...
public class URLab : ModuleRules
{
	...
	// layout are recorded in docs/ros2_link_facts.md from an actual Lyrical
	// install; the standalone harness (ros/urlab_ros_ws) validates the same C
	// code against real DDS.
	protected void AddRos2(ReadOnlyTargetRules Target)
	{
		...
		if (string.IsNullOrEmpty(RosRoot) || !Directory.Exists(RosRoot))
		{
			...
			Console.WriteLine("URLab: ROS 2 not found (set URLAB_ROS2_ROOT to enable) - building without ROS.");
			return;
		}
		...
		if (Target.Platform == UnrealTargetPlatform.Win64)
		{
			foreach (string Name in LinkNames)
			{
				...
				{
					Console.WriteLine("URLab: ROS 2 lib not found (skipped): {0}", LibFile);
				}
			}
		...
		else if (Target.Platform == UnrealTargetPlatform.Linux)
		{
			foreach (string Name in LinkNames)
			{
				...
				// an absolute versioned path (as AddThirdPartyLibrary does).
				...
				{
					Console.WriteLine("URLab: ROS 2 lib not found (skipped): {0}", SoFile);
				}
			}
	...
	// AddThirdPartyLibrary. De-duplicates so overlapping patterns stage once.
	private void StageRosRuntime(string Dir, string[] Patterns, bool bDelayLoad)
	{
		if (!Directory.Exists(Dir))
		{
			Console.WriteLine("URLab: ROS 2 runtime dir not found (no staging): {0}", Dir);
			return;
		}

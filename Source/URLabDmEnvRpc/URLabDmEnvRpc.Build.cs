...
public class URLabDmEnvRpc : ModuleRules
{
	public URLabDmEnvRpc(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		{
			// gRPC installs Unix static archives (.a) or MSVC import/static libs (.lib)
			// depending on the platform it was built for.
			string LibPattern = Target.Platform == UnrealTargetPlatform.Win64 ? "*.lib" : "*.a";
			foreach (string LibFile in Directory.GetFiles(GrpcLib, LibPattern))
			{
		...
		{
			// gRPC / boringssl / cares Windows link deps. Tune after the Windows build
			// reports its exact undefined symbols.
			PublicSystemLibraries.AddRange(new string[] { "ws2_32", "crypt32", "advapi32" });
		}
	}
}

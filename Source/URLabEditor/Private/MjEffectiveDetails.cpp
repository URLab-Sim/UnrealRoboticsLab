...
void FMjEffectiveDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	for (FOptionalProperty* const Optional : Unset)
	{
		...
		// Scoped to the declaring class first, then unscoped. The scoped form is
		// the precise one and is what the row tests exercise, but it asks for a
		// property under the class that DECLARES it, and those differ whenever an
		// element has a hand subclass: a joint is `UMjJoint` and matches, a geom's
		// attributes are declared on `UMjGeomBase` while the object is `UMjGeom`.
		// In a Blueprint's details panel that mismatch returned an invalid handle
		// and every geom attribute was skipped by the `continue` below -- silently,
		// which is why the panel showed bare Set buttons while this function ran
		// and found twenty-five unset attributes to describe.
		TSharedPtr<IPropertyHandle> Handle =
			DetailBuilder.GetProperty(Optional->GetFName(), Optional->GetOwnerClass());
		if (!Handle.IsValid() || !Handle->IsValidHandle())
		{
			Handle = DetailBuilder.GetProperty(Optional->GetFName());
		}
		if (!Handle.IsValid() || !Handle->IsValidHandle())
		{
			continue;

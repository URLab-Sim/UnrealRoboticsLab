	...
	FString& OutLevelPath, FString& OutError)
{
	...
	if (!Sub)
	{
		OutError = TEXT("LevelEditorSubsystem unavailable");
		return false;
	}
...
bool DestroyAssetSync(const FString& ObjectPath, bool& bOutWasFound, FString& OutError)
{
	...
	if (CurWorld && CurWorld->GetOutermost() == Existing.GetPackage())
	{
		ULevelEditorSubsystem* Sub = GetLevelSub();
		if (Sub)
			Sub->LoadLevel(TEXT("/Engine/Maps/Templates/Template_Default"));
	}
...
bool ShouldListActor(AActor* A)
{
	...
	if (A->IsA(ABrush::StaticClass()))
		return false;
	if (A->GetClass()->GetName().Contains(TEXT("WorldSettings")))
		return false;
	if (A->GetClass()->GetName().Contains(TEXT("DefaultPhysicsVolume")))
		return false;
	if (A->bHiddenEdLayer)
		return false;
	// Editor-only / engine actors that clutter the list.
	const FString ClassName = A->GetClass()->GetName();
	if (ClassName.StartsWith(TEXT("AbstractNavData")))
		return false;
	if (ClassName == TEXT("WorldDataLayers"))
		return false;
	if (ClassName == TEXT("LevelInstanceEditorInstanceActor"))
		return false;
	if (ClassName == TEXT("WorldPartition"))
		return false;
	return true;
}
} // namespace
	...
	FString& OutError)
{
	...
	if (!QC)
	{
		...
		if (!QC)
		{
			OutError = TEXT("NewObject UMjQuickConvertComponent failed");
			return false;
		}
	...
	FString& OutError)
{
	...
	for (const FAssetData& Data : Assets)
	{
		...
		if (!Data.GetTagValue(ParentClassTag, ParentClassExportPath))
			continue;
		...
		if (!ParentClass)
			continue;
		if (!ParentClass->IsChildOf(AMjArticulation::StaticClass()))
			continue;
	...
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		...
		if (!ClassFilter.IsEmpty())
		{
			...
			for (UClass* C = A->GetClass(); C != nullptr; C = C->GetSuperClass())
			{
				...
				if (N.Equals(ClassFilter, ESearchCase::IgnoreCase) || (!FilterStripped.IsEmpty() && N.Equals(FilterStripped, ESearchCase::IgnoreCase)))
				{
					bMatch = true;
					break;
				}
	...
	FString& OutError)
{
	...
	BuildNode = [&BuildNode](AActor* N) -> TSharedPtr<FJsonObject> {
		...
		for (AActor* C : Attached)
			if (C)
				Children.Add(MakeShared<FJsonValueObject>(BuildNode(C)));

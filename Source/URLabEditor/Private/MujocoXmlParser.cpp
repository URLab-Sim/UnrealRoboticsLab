...
static FString ResolveMaterialFromDefaults(const UMjGeom* GeomComp, UBlueprint* BP)
{
	...
	if (GeomComp->bOverride_material && !GeomComp->material.IsEmpty())
		return GeomComp->material;
	...
	USCS_Node* ReuseNode)
{
	...
	else if (Tag.Equals(TEXT("frame")))
	{
		...
		if (ParentNode)
			ParentNode->AddChildNode(CreatedNode);
		else
			BP->SimpleConstructionScript->GetDefaultSceneRootNode()->AddChildNode(CreatedNode);
	...
	else if (Tag.Equals(TEXT("geom")))
	{
					...
					for (USCS_Node* DefChild : DefNode->ChildNodes)
					{
						...
						switch (DefGeom->Type)
						{
							...
							case EMjGeomType::Capsule:
								TypeStr = TEXT("capsule");
								break;
							case EMjGeomType::Cylinder:
								TypeStr = TEXT("cylinder");
								break;
		...
		if (TypeStr == "box")
			Class = UMjBox::StaticClass();
		else if (TypeStr == "sphere")
			Class = UMjSphere::StaticClass();
	...
	else if (Tag.Equals(TEXT("joint")))
	{
		...
		if (TypeStr == "hinge")
			Class = UMjHingeJoint::StaticClass();
		else if (TypeStr == "slide")
			Class = UMjSlideJoint::StaticClass();
		else if (TypeStr == "ball")
			Class = UMjBallJoint::StaticClass();
		else if (TypeStr == "free")
			Class = UMjFreeJoint::StaticClass();
		...
		if (JointComp)
		{
			...
			if (!ClassAttr.IsEmpty() && CreatedDefaultNodes.Contains(ClassAttr))
			{
				UMjDefault* DefComp = Cast<UMjDefault>(CreatedDefaultNodes[ClassAttr]->ComponentTemplate);
				if (DefComp)
					JointComp->DefaultClass = DefComp;
			}
		}
	}
	...
	else if (Tag == "motor" || Tag == "position" || Tag == "velocity" || Tag == "cylinder" || Tag == "muscle" || Tag == "general" || Tag == "damper" || Tag == "intvelocity" || Tag == "adhesion" || Tag == "dcmotor")
	{
		...
		if (Tag == "motor")
			Class = UMjMotorActuator::StaticClass();
		else if (Tag == "position")
			Class = UMjPositionActuator::StaticClass();
		else if (Tag == "velocity")
			Class = UMjVelocityActuator::StaticClass();
		else if (Tag == "muscle")
			Class = UMjMuscleActuator::StaticClass();
		else if (Tag == "cylinder")
			Class = UMjCylinderActuator::StaticClass();
		else if (Tag == "damper")
			Class = UMjDamperActuator::StaticClass();
		else if (Tag == "intvelocity")
			Class = UMjIntVelocityActuator::StaticClass();
		else if (Tag == "adhesion")
			Class = UMjAdhesionActuator::StaticClass();
		else if (Tag == "dcmotor")
			Class = UMjDcMotorActuator::StaticClass();
		else if (Tag == "general")
			Class = UMjGeneralActuator::StaticClass();
	...
	if (CreatedNode)
	{
		if (ParentNode)
			...
			BP->SimpleConstructionScript->GetDefaultSceneRootNode()->AddChildNode(CreatedNode); // Fallback attach to root
...
void UMujocoGenerationAction::ParseAssetsRecursive(const FXmlNode* Node, const FString& XMLDir, TMap<FString, FString>& OutMeshAssets, TMap<FString, FVector>& OutMeshScales, TMap<FString, FString>& OutTextureAssets, TMap<FString, FMuJoCoMaterialData>& OutMaterialData, const FString& MeshDir, const FString& TextureDir, const FString& AssetDir)
{
	...
	if (Tag.Equals(TEXT("mujoco")) || Tag.Equals(TEXT("include")) || Tag.Equals(TEXT("asset")))
	{
		for (const FXmlNode* Child : Node->GetChildrenNodes())
		{
			if (Child->GetTag().Equals(TEXT("compiler")))
			{
				...
				if (!AttributeMeshDir.IsEmpty())
					CurrentMeshDir = AttributeMeshDir;

				FString AttributeTextureDir = Child->GetAttribute(TEXT("texturedir"));
				if (!AttributeTextureDir.IsEmpty())
					CurrentTextureDir = AttributeTextureDir;

				FString AttributeAssetDir = Child->GetAttribute(TEXT("assetdir"));
				if (!AttributeAssetDir.IsEmpty())
					CurrentAssetDir = AttributeAssetDir;
	...
	else if (Tag.Equals(TEXT("mesh")))
	{
		...
		{
			if (MeshName.IsEmpty())
				MeshName = FPaths::GetBaseFilename(MeshFile);
	...
	else if (Tag.Equals(TEXT("texture")))
	{
		...
		{
			if (TexName.IsEmpty())
				TexName = FPaths::GetBaseFilename(TexFile);
	...
	else if (Tag.Equals(TEXT("material")))
	{
		...
		if (!MatName.IsEmpty() && !OutMaterialData.Contains(MatName))
		{
			...
			{
				if (!ChildNode || !ChildNode->GetTag().Equals(TEXT("layer")))
					continue;
				FString LayerTex = ChildNode->GetAttribute(TEXT("texture"));
				FString LayerRole = ChildNode->GetAttribute(TEXT("role"));
				if (LayerTex.IsEmpty() || LayerRole.IsEmpty())
					continue;
				LayerRole = LayerRole.ToLower();
				if (LayerRole == TEXT("rgb"))
					MatData.BaseColorTextureName = LayerTex;
				else if (LayerRole == TEXT("normal"))
					MatData.NormalTextureName = LayerTex;
				else if (LayerRole == TEXT("orm"))
					MatData.ORMTextureName = LayerTex;
				else if (LayerRole == TEXT("roughness"))
					MatData.RoughnessTextureName = LayerTex;
				else if (LayerRole == TEXT("metallic"))
					MatData.MetallicTextureName = LayerTex;
				else if (LayerRole == TEXT("occlusion"))
					MatData.ORMTextureName = LayerTex; // best-effort
				else
				{
...
void UMujocoGenerationAction::ParseDefaultsRecursive(const FXmlNode* Node, UBlueprint* BP, USCS_Node* RootNode, const FString& XMLDir, const FMjCompilerSettings& CompilerSettings, const FString& ParentClassName, bool bIsDefaultContext)
{
	...
	else if (Tag.Equals(TEXT("default")))
	{
		...
		for (const FXmlNode* Child : Node->GetChildrenNodes())
		{
			...
			// Actuators
			else if (ChildTag.Equals(TEXT("motor")) || ChildTag.Equals(TEXT("position")) || ChildTag.Equals(TEXT("velocity")) || ChildTag.Equals(TEXT("cylinder")) || ChildTag.Equals(TEXT("muscle")) || ChildTag.Equals(TEXT("general")) || ChildTag.Equals(TEXT("damper")) || ChildTag.Equals(TEXT("actuator")) || ChildTag.Equals(TEXT("adhesion")) || ChildTag.Equals(TEXT("intvelocity")) || ChildTag.Equals(TEXT("dcmotor")))
			{
				...
				if (ChildTag == "motor")
					ActClass = UMjMotorActuator::StaticClass();
				else if (ChildTag == "position")
					ActClass = UMjPositionActuator::StaticClass();
				else if (ChildTag == "velocity")

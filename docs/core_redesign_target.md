...
| 10 DR override channel | subagent | impl (awaiting build) | proto/entity-redesign | UMjAppearanceStore (manager-owned, name-keyed) + resolve_geom/set_geom_appearance RPCs (scene ns) driving MjAppearance::Apply in BOTH render paths (UMjGeom + AMjbScene); textures via existing asset-cache/asset-path; clear restores authored. Re-apply wired for fast-path reload. GAP: authoring RebuildVisualizer recompile drops overrides (ReapplyAll not hooked there yet). Visual result needs LIVE RPC camera validate (after user reboot). |
